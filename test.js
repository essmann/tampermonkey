  ...
  class Config {
    ...
    static categoryHandlers = {
      font: {
        ...
        default: ()=>document.body.style.fontFamily = ""
      },
      language: {
        handler: (value)=> console.log("lang handler!"),
        default: ()=>console.log("default  lang handler!")
      }
    }
  ...
  class CommandBuilder {
    static commandTemplate (command) {
      ...
      `
        ...
        command.id
      }" style="visibility: ${command.enabled ? 'visible' : 'hidden'}"></div>
          <div class="commandName">${command.name}</div>
        </div>
      `
